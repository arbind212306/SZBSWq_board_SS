...
class SupervisorController extends AppController {
    ...
    public function dashboard() {
        ...
        $query1 = $userTbl->find('all')
                ->where(['user_type' => 4, 'status !=' => 0, 'ob_status' => 1,'manager_emp_id'=>$user['emp_id']])
...
//        pr($asnData);die;
    ...
    public function getJoinee($user_id) {
        ...
        $userTbl = TableRegistry::getTableLocator()->get('Users');
        $query1 = $userTbl->find('all')
                ->where(['Users.id' => $user_id, 'ob_status' => 1, 'Users.status !=' =>0])
...
//    }
    ...
    public function getFeedbackData($user_id) {
        ...
        $todayD=Time::now();
        $diff = date_diff($todayD, $uinfo['doj']);
        ...
        foreach ($query1 as $rw) {
            ...
            if (!empty($rw)) {
                ...
                // Allow feedback
                $diff_class="panel-success info-sec";
                $showFeedbckFrm=true;
                if($rw['interval_days']>=$diff){
                    $diff_class="panel-danger disabled-pointer";
                    $showFeedbckFrm=false;
                }
                //var_dump($diff);die;
                //var_dump($showFeedbckFrm);die;
                //disabled-pointer

                // Html for info
                $infohtml = '<div class="col-md-4">
                                <div class="panel '.$diff_class.' joinee-blocks" data-rel="' . $key . '">
