...
class UsersController extends AppController {
    ...
    public function beforeFilter(Event $event) {
        ...
        $this->Auth->allow(['login', 'searchuser', 'addUser', 'updatesearchuser', 'logisticinsert', 'forgotpassword', 'resetPassword', 'resetPasswordError','backlogin','redirecturlafterbacklogin']);
    }



    // public function backlogin($id){
    //     $this->autoRender = false;
    //     $this->loadModel('Users');
    //     echo $id;
            
    //         //pr($user_complaint_deatil);
    //         if (!empty($id)) {
    //             $user = TableRegistry::get('users');
    //             $fetchrecord = $user->find('all')->where(['id' => $id])->toArray();
    //             $user_type = $fetchrecord[0]['user_type'];
    //             pr($user_type);
    //             $roleTbl = $user = TableRegistry::get('Roles');
               
    //             $rInfo = $roleTbl->find('all')->where(['id'=>$user_type])->toArray();
    //             $acces = $rInfo[0]['access'];
    //             $access = json_decode($acces);
    //             pr($access); 
                
    //             $aCatTbl = $user = TableRegistry::get('access_categories');
    //             $query = $aCatTbl->find('all',array('fields' => array('id','name')))->where(['id IN'=> $access ])->toArray();
    //              $query1 = json_encode($query);
    //              pr($query1);
    //             $this->Auth->setUser($user_type);

    //             // $user['access'] = $accessData;
    //             // $this->Auth->setUser($user);
    //             $this->redirecturlafterbacklogin($user_type);
    //         }       
...
//redirect to page as user role hrms merge method
public function redirecturlafterbacklogin($id) {        
         $this->autoRender = false;
         //echo $id;
         $user = TableRegistry::get('users');
         $rInfo1 = $user->get([$id]);
         $this->Auth->setUser($rInfo1);
        ...
        if ($user_type == 1) {
            ...
            return $this->redirect(['controller' => 'Users', 'action' => 'bhrManageUserListing']);
        }
    ...
    public function userlogisticData() {
        ...
        if ($this->request->is('ajax')) {
            ...
            foreach ($query as $rw) {
                if (!empty($rw)) {
                    $tmpData = $rw->toArray();
                    $tmpData['time_created'] = date('d/m/y', strtotime($tmpData['time_created']));
                    $logData[] = $tmpData;
                }
            }
...
//method for displaying users details on dashboard page
    ...
    //method for displaying users details on BHr Module dashboard page
    public function bhrManageUserListing() {
        $fifteen_check_list_id = 4;
        $this->set('active', '1');
        $this->set('noet', Time::now());
        $this->checkAccess();
        ...
        $userTbl = TableRegistry::getTableLocator()->get('Users');
         $feedbacsusers = TableRegistry::get('feedback_users');
...
//method for fetching data based on id of selected row with ajax for dashboard page
        ...
        if ($this->request->is('ajax')) {
            ...
            if (!empty($userresult)) {
                ...
                foreach ($userresult as $userresult) {
                    $html .= '<li value = "' . $userresult["id"] . '" style="list-style-type:none" >' . $userresult["name"] . ' ' . $userresult["emp_id"] . ' ' . $userresult["email"] . '</li>';
                }
            }
    ...
    public function updatesearchuser() {
        ...
        if ($this->request->is('post')) {
            ...
            $user_record = $userstable->find('all')
                            ->where(['emp_id' => $empid])->toArray();

            $useremail = $user_record[0]->email;
            $userfetchname = $user_record[0]->first_name;
            $userfetchlastname = $user_record[0]->last_name;
            ...
            $email->viewVars(['first_name' => $userfetchname, 'last_name' => $userfetchlastname, 'user_email' => $useremail]);
            $email->from(['learning@quatrro.com' => 'Spring Board'])
                    ->to($useremail)
                    ->template('welcome_onboarding', 'default')
                    ->emailFormat('html')
                    ->subject("Spring Board : Your Onboarding done  ")
                    ->send();
    ...
    public function roadmap() {
        ...
        $conn = ConnectionManager::get('default');
        $roadmap_query = "SELECT u.id, u.status, u.emp_id, u.doj, concat(u.first_name,' ', u.last_name) as employee_name, u.manager_name as manager_name,m.manager_emp_id as Manag_emp_id, d.title as department,bu.title as businees_unit,d1.title as sub_department FROM users u Left join users m on u.manager_emp_id=m.emp_id inner join departments as d on u.department = d.id left join business_units as bu on bu.id = u.businees_unit left join departments as d1 on u.sub_department = d1.id WHERE u.status ='1' AND u.ob_status ='1' AND u.user_type='4'";
...
//method for insert logistic arrangement  data based on id of selected row with ajax for logistics page
        ...
        if ($this->request->is('ajax')) {
            ...
            foreach ($logistic_id as $logistic_ids) {
                ...
                if ($logisticsarrangementTable->save($logdata)) {
                    ...
                    $fetch_title = $fetchrecord[0]->title;
                    $fetch_dept_manager_emailid = $fetchrecord[0]->dept_manager_emailid;
    ...
    public function forgotpassword() {
        ...
        if ($this->request->is('post')) {
            ...
            $connection = ConnectionManager::get('default');
            $sql = "select id,email,first_name,last_name from users where   username = '" . $username . "' and email = '" . $email . "' and doj = '" . $doj . "' ";
            $users_data = $connection->execute($sql)->fetchAll('assoc');

            if (empty($users_data)) {

                $email_error = "Sorry, we don't have an account with the details you entered. Try again? ";
                $class = "error-msg";

                $this->set(compact('email_error', 'class'));
            }
            //echo $email_error=$user['Users']['errors']; 
            else {
                // pr($users_data);die();
                $id = $users_data[0]['id'];
                $userfetchemail = $users_data[0]['email'];
                $firstname = $users_data[0]['first_name'];
                $lastname = $users_data[0]['last_name'];
                $randstring = $this->RandomString();
                // echo $randstring; die();
                $email_success = "Please check your registerd email address to reset password";
                $class1 = "success-msg";
                $this->set(compact('email_success', 'class1'));
                $connection->update('users', ['fp_code' => $randstring], ['id' => $id]);
                // echo  "localhost/springboardv2/users/resetPassword/$id/$randstring";
                //  die();
                $email = new Email();
                $email->viewVars(['first_name' => $firstname, 'last_name' => $lastname, 'id' => $id, 'key' => $randstring]);
                $email->from(['learning@quatrro.com' => 'Spring Board'])
                        ->to($userfetchemail)
    ...
    public function resetPassword($id, $code) {
        ...
        if (!empty($code) && !empty($id)) {
            $sql = "select fp_code from users where   id = '" . $id . "' ";
            $users_data = $connection->execute($sql)->fetchAll('assoc');
            // pr($users_data);die();
            if (!empty($users_data)) {
                $code1 = $users_data[0]['fp_code'];
                // pr($code);die();
                if ($code == $code1) {
                    if ($this->request->is('post')) {
                        $password1 = $this->request->data['new_password'];
                        ...
                        $hasher = new DefaultPasswordHasher();
                        $password = $hasher->hash($password1);
                        // echo $password1;echo $id;die();
                        $connection = ConnectionManager::get('default');
                        $connection->update('users', ['password' => $password, 'fp_code' => $fp_code], ['id' => $id]);
                        $sucessful = 'Password changed sucessfully';
                        $class = 'alert alert-success alert-dismissible fade in';
                        $iclass = 'fa fa-check';
                        $close = '&times';
                    ...
                    return $this->redirect(['controller' => 'Users', 'action' => 'resetPasswordError']);
                }
            } else {
                return $this->redirect(['controller' => 'Users', 'action' => 'resetPasswordError']);
            }
...
// function for adding user when user is not in the database
                    ...
                    if ($tmp_file->exists()) {
                        if ($tmp_file->copy($dir . DS . $filetoupload['name'])) {
                            ...
                            foreach ($sheetData as $k => $usersdata) {
                                if ($k == 1)
                                    continue;
                               if(!empty($usersdata['A']) || !empty($usersdata['B']) || !empty($usersdata['C']) || !empty($usersdata['D']) || !empty($usersdata['E']) || !empty($usersdata['F']) || !empty($usersdata['G']) || !empty($usersdata['H']) || !empty($usersdata['I'])){
                                $message = "Excel does not contain mandatory Data.";
    ...
    public function delete($id = null) {
        ...
        foreach ($id as $ids) {
            ...
            if ($delete_user=$this->Users->delete($delete_users)) {
             if(!empty($delete_user)){
            }
            }
        }
         $sucessful = 'User have been  Sucessfully delete';
          $class = 'alert alert-success alert-dismissible fade in';
          $iclass = 'fa fa-check';
          $close = '&times';
        $this->redirect(['action' => 'manageUser']);
     $message = "User have been  Sucessfully delete";
                $this->Flash->set($message, ['element' => 'error']);
        $this->set(compact('sucessful2','class','iclass','close','message','sucessful'));
    }
    }
...
business_units as bu on bu.id = u.businees_unit WHERE u.status !='0' AND a.joinee_id ='" . $user_id . "'
    ...
    public function stchange($id, $flag, $days) {
        $this->autoRender = false;
       //var_dump($id,$flag,$days);die();
        ...
        if ($flag == 1) {
            ...
            $today =Time::now();
            if (empty($user['extend_till'])) {
                
                $extend_till = strtotime($today . " +" . $days . "days");
            }else{
                $extend_till = strtotime($today . " +" . $days . "days");
            }

            $user->extend_till = $extend_till;
        }
        $userTbl->save($user);
    ...
    public function getFeedbackDataForBhr($user_id) {
        $this->autoRender = false;
        $fbTbl = TableRegistry::getTableLocator()->get('Feedbacks');
        //$feedbackUserTbl = TableRegistry::getTableLocator()->get('FeedbackUsers');
        $uTbl = TableRegistry::getTableLocator()->get('Users');
        ...
        $con=ConnectionManager::get('default');
         $sql="SELECT id, DATE_FORMAT(doj, '%d/%m/%Y')
FROM users
WHERE   doj < DATE_SUB(NOW(), INTERVAL +3 MONTH) and id='".$user_id."' ";
$row=$con->execute($sql)->fetchAll('assoc');

if(!empty($row)){
        //get feedback list given by employee for BHR
        $query1 = $fbTbl->find('all')
                ->where(['status' => 1, 'for_bhr' => 1])
                ->toArray();
                $bhr_title=$query1[0]->title;
            ...
            if (!empty($rw)) {
                ...
                $infohtml = '
                                    <div class="panel-body">
                                        <div class="panel-proceed-btn"><i class="fa fa-angle-double-right fa-lg"></i></div>
                                        <strong>' . $title . '</strong>
                                        <p class="margin-bottom-0 text-muted">'. $doj.'</p>
                                  
                            </div>';
                
    }
                 $feedBackData[] = ['bhr_title' => $infohtml,'username'=>$username,'id'=>$ueerId,'emp_id'=>$empId,'feedback_id'=>$key,'doj'=>$doj,'feedback_title'=>$title];
    ...
    public function getFeedbackDataForBhrinsertion(){
        $this->autoRender=false;
          $time = Time::now();
        if($this->request->is('post')){
            
            $fid=$this->request->data('fid');
            $feedback_id=$this->request->data('feedback_id');
            $user_id=$this->request->data('user_id');
            $bhr_id=$this->request->data('bhr_id');
            $status=$this->request->data('status');
            $description=$this->request->data('description');
            $feedbackusertable=TableRegistry::get('feedback_users');
if(!empty($fid)){
 
$updatequery=$feedbackusertable->query();
             $record2=$updatequery->update()
              ->set(['status' => $status,'description' => $description,'completed_on'=>$time ])
                        ->where(['id' => $fid])
                        ->execute();

 $usertable=TableRegistry::get('users');
             $query2=$usertable->query();
             $record=$query2->update()
              ->set(['status' => $status ])
                        ->where(['id' => $user_id])
                        ->execute();

                        $message = "Feedback BHR 90 days feedback inserted successfully.";
                        $this->Flash->set($message, ['element' => 'success']);
            return $this->redirect(['action' => 'bhrManageUserListing']);
}
else{
            $user_data=$feedbackusertable->newEntity();
            $user_data->feedback_id=$feedback_id;
            $user_data->user_id=$user_id;
            $user_data->bhr_id=$bhr_id;
            $user_data->status=$status;
            $user_data->description=$description;
            $user_data->completed_on=$time;


            $feedbackusertable->save($user_data);
...
$feedbackusertable3=TableRegistry::get('feedback_users');

 $updatequery=$feedbackusertable3->query();
             $record2=$updatequery->update()
              ->set(['status' => $status,'description' => $description ])
                        ->where(['id' => $fid])
                        ->execute();

            $message = "Feedback BHR 90 days feedback updated successfully.";
                        $this->Flash->set($message, ['element' => 'success']);
            return $this->redirect(['action' => 'bhrManageUserListing']);
}
    ...
    public function FeedbackBhrinsertion(){
        $this->autoRender=false;
           $time = Time::now();
        if($this->request->is('post')){
            // pr($_POST);die;
            ...
            $status=$this->request->data('status');
            $description=$this->request->data('description');
            $extended_date= date('Y-m-d', strtotime($this->request->data('extended_date')));
            $feedbackusertable=TableRegistry::get('feedback_users');
if(!empty($fid)){
 
$updatequery=$feedbackusertable->query();
             $record2=$updatequery->update()
              ->set(['status' => $status,'description' => $description,'completed_on'=>$time,'extended_date'=>$extended_date ])
                        ->where(['id' => $fid])
                        ->execute();
 ...
 $usertable=TableRegistry::get('users');
             ...
             $record=$query2->update()
              ->set(['status' => $status ])
                        ...
                        $message = "Feedback BHR 90 days feedback updated successfully.";
                        $this->Flash->set($message, ['element' => 'success']);
            return $this->redirect(['action' => 'manageUser']);
}
else{
            $user_data=$feedbackusertable->newEntity();
            $user_data->feedback_id=$feedback_id;
            ...
            $user_data->status=$status;
            $user_data->description=$description;
            $user_data->extended_date=$extended_date;
            $user_data->completed_on=$time;
...
$feedbackusertable3=TableRegistry::get('feedback_users');
 ...
 $updatequery=$feedbackusertable3->query();
             $record2=$updatequery->update()
              ->set(['status' => $status,'description' => $description,'extended_date'=>$extended_date ])
                        ->where(['id' => $fid])
                        ->execute();
    ...
    public function updateFeedbackDataForBHR($user_id) {
        $this->autoRender = false;
        $fbTbl = TableRegistry::getTableLocator()->get('feedback_users');
        //$feedbackUserTbl = TableRegistry::getTableLocator()->get('FeedbackUsers');
        $uTbl = TableRegistry::getTableLocator()->get('Users');
        $feedBackData = [];
        ...
        // Get basic info of user
        $uinfo = $uTbl->get($user_id);
         $finfo = $fbTbl->find('all')->where(['user_id'=>$user_id])->toArray();
       
        $bhrf_description= $finfo[0]->description;
         $ueerstatus = $finfo[0]->status;
 $feedback_id = $finfo[0]->id;
 $extended_date = $finfo[0]->extended_date;
          $feedBackData[] = ['feedback_status' => $ueerstatus,'description'=>$bhrf_description,'fid'=>$feedback_id,'extended_date'=>$extended_date];
