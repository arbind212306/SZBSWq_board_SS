                ...
                <div class="tab-content">
                    ...
                    <div id="interval_feedback" class="tab-pane fade">
                        <div id="interval_feedback_outer">
							...
							<div class="row" style="margin-left: 1px;margin-bottom: 9px;">
                                <strong>Feedback by BHR</strong>
                            </div>
                            <div class="row" >
                                            <div class="col-md-4">
                                <div class="panel panel-success joinee-blocks " id="bhr_title" onclick="BHRnightyFeedback()"     data-rel="">
                            </div>
                        ...
                        <!--<div id="" class="hidden">
                            <ol class="breadcrumb">
                                <li class="text-blue pointer" onclick="">Interval Feedback</li>
						 ...
						 <div id="BHR_nighty_feedback"  style="display:none;" >
                            <ol class="breadcrumb">
                                <li class="text-blue pointer" onclick="internalfeedback();">Internal Feedback</li>
                                <li class="active" id="feedback_title"></li>
                            </ol>
                            ...
                            <div class="row">
								...
								$bhruser_id=$session_data["Auth"]["User"]["id"];
								
								?>
							<?php //echo $this->Form->create('', array('url' => array('controller' => 'Users', 'action' => 'getFeedbackDataForBhrinsertion'))); ?>
								
                                  <?php echo $this->Form->create('', array('url' => array('controller' => 'Users', 'action' => 'getFeedbackDataForBhrinsertion'))); ?>
                                        <div class="row">
                                            <div class="col-md-12">
                                                <input class="form-control" id="fid" type="hidden" name="fid" >
                                                <input class="form-control" id="feedback_id" type="hidden" name="feedback_id" >
                                          <input class="form-control" id="buserid" type="hidden" name="user_id" >
										    <input class="form-control" id="bhr_id" type="hidden" name="bhr_id" value="<?= $bhruser_id;?>">
                                                <div class="form-group row">
                                                    <div class="col-md-4">
                                                        <label for="90_session_interval" class="padding-top-md">Employee Status</label>
                                                    </div>
                                                    <div class="col-md-8 bu_name_es_ap">
                                                ...
                                                <div class="form-group row">
                                                    <div class="col-md-4">
                                                        <label for="90_session_comments" class="padding-top-md">Description</label>
                                                    </div>
                                                    <div class="col-md-8">
                                                ...
                                                <div class="form-group row">
												 <div class="col-md-4"></div>
                                                    <div class="col-md-8 text-right">
                                                        <button class="btn btn-danger btn-block" id="bhr_feedbackinsert">Submit</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
...
<div class="container-fluid container-padding-top">
    ...
    <?php if (!empty(@$flash_message)) { ?>
        <div class="success-msg" id="success" style="text-align:center;">
           ...
           Status have been  sucessfully changed.
        </div>
    <?php } ?>-->
            ...
            <table class="table table-bordered table-striped" id="myTable">
                <thead>
                    <tr>
                        ...
                        <th><div data-toggle="tooltip" data-placement="bottom" data-original-title="Status of 45 Day BHR Connect" class="red-tooltip">Confirmation</div></th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                        ...
                        foreach ($users as $showUser):
                            ...
                            <tr data-type="completed">
                                ...
                                <td><div class="text-blue logistic_user_arrangements" data-key="<?= $showUser['id'] ?>" data-sidebar-button id="<?= 'user_' . $showUser['id'] ?>" onclick="displayIdBasedData('<?= $showUser['id'] ?>');getsessions('<?= $showUser['id'] ?>');">
                                        ...
                                        <td>
                                            <?php
                                            $doj = date_create($showUser['doj']);
                                            $today = $noet;
                                            $diff = date_diff($today, $doj);
...
//                                            var_dump($doj);
//                                            var_dump($diff);die;
                                            if ($showUser['status'] == 2) {
                                                echo '<span class="badge badge-success"><i class="fa fa-check"></i> Confirmed</span>';
                                            } else {
                                                if ($diff > 89) {
                                                    // If already extended
                                                    if (!empty($showUser['extend_till'])) {
                                                        $extnd = date_create($showUser['extend_till']);
                                                        $diff2 = date_diff($today,$extnd);
                                                        $diff2 = intval($diff2->format("%R%a"));
                                                       // var_dump($diff2);
                                                        if ($diff2 > 1) {
                                                            echo '<span class="badge badge-info">&nbsp; Extended</span>';
                                                        } else {
                                                            echo '<button class="btn btn-success btn-sm u_confirm" data-key="' . $showUser['id'] . '">Confirm</button>';
                                                            echo '&nbsp;<button type="button" class="btn btn-danger btn-sm u_reject" data-key="' . $showUser['id'] . '">Extend</button>';
                                                        }
                                                    } else {
                                                        echo '<button class="btn btn-success btn-sm u_confirm" data-key="' . $showUser['id'] . '">Confirm</button>';
                                                        echo '&nbsp;<button type="button" class="btn btn-danger btn-sm u_reject" data-key="' . $showUser['id'] . '">Extend</button>';
...
$bhr_data = $this->Url->build(['controller' => 'Users', 'action' => 'getFeedbackDataForBhrinsertion']);
?>
<script>
...
<script>
  function BHRnightyFeedback(){
                     
	$('#BHR_nighty_feedback').show();
	$('#interval_feedback_outer').hide();
	
  }
        function internalfeedback(){
                  	$('#BHR_nighty_feedback').hide();
	$('#interval_feedback_outer').show();
...
<script>
 ...
 $(document).ready(function(){
	 $('bhr_feedbackinsert').click(function(){ 
	 var feedback_id=$('#feedback_id').val();
	 var buserid=$('#buserid').val();
	 var bhr_id=$('#bhr_id').val();
	 var bstatus=$('#bstatus').val();
	 var description=$('#description').val();
	  $.ajax({
            url: "<?= $bhr_data; ?>",
            type: 'POST',
            data: {"feedback_id": feedback_id, "buserid": buserid,"bhr_id":bhr_id, "bstatus": bstatus,"description": description},
...
<script>
    ...
    $(document).on('click', '.u_reject', function () {
        var r = prompt("Enter  number of  days ");
        if (isNaN(r) || (r == '0')) {
        ...
        var key = $(this).attr('data-key');
        $.ajax({
            url: webroot + 'users/stchange/' + key + '/0/' + r
        }).done(function () {
            btn.parent().html('<span class="badge badge-info">&nbsp; Extended</span>');
        });
    });
</script>
<style>
.form-control.input-sm {
 ...
 /*.pagination > li > a, .pagination > li > span{background-color:#d9534f}
.pagination > li.active > a, .pagination > li.active > span{background-color:#d9534f}*/
 /*.pagination > .active > a{
    background-color:#d9534f;
    border-color: #d9534f;
 }
.pagination > li > a:hover, .pagination > li > span:hover, , .pagination > li > span:active, .pagination > li > a:focus, .pagination > li > span:focus{
    background-color:#d9534f;
    border-color: #d9534f;
}*/
